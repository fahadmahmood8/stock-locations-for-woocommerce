            ...
            if( $update ) {
                ...
                foreach ( $order->get_items() as $item_id => $item ) {
                    ...
                    if( $item_product->get_type() === 'variation' ) {
                        ...
                        $item_stock_location_terms = SlwStockAllocationHelper::getProductStockLocations($product->get_ID(), true, null);
                    } else {
                        // Get item location terms
                        $item_stock_location_terms = SlwStockAllocationHelper::getProductStockLocations($item_product_id, true, null);
                    }
                    ...
                    if( isset($item_stock_location_terms) && !empty($item_stock_location_terms) ) {
                        ...
                        foreach ( $item_stock_location_terms as $term ) {
                            $input_quantities[] = sanitize_text_field($_POST[SLW_PLUGIN_SLUG . '_oitem_' . $item_product_id . '_' . $term->term_id]);
                        }
                        $input_quantities = array_sum($input_quantities);

                        // Get post meta
                        $postmeta_stock = get_post_meta($item_product_id, '_stock', true);
                        $postmeta_stock_status = get_post_meta($item_product_id, '_stock_status', true);
                        ...
                        foreach ( $item_stock_location_terms as $term ) {
                            ...
                            if( isset($_POST[SLW_PLUGIN_SLUG . '_oitem_' . $item_product_id . '_' . $term->term_id]) ) {

                                // Initiate counter
                                $counter++;

                                $item_stock_location_subtract_input_qty = sanitize_text_field($_POST[SLW_PLUGIN_SLUG . '_oitem_' . $item_product_id . '_' . $term->term_id]);
                                $postmeta_stock_at_term = get_post_meta($item_product_id, '_stock_at_' . $term->term_id, true);
                                ...
                                if( ($item_stock_location_subtract_input_qty !== 0) && !empty($item_stock_location_subtract_input_qty) && ($item_stock_location_subtract_input_qty <= $item_quantity) && ($input_quantities === $item_quantity) ) {
                                    ...
                                    update_post_meta( $item_product_id, '_stock_at_' . $term->term_id, $postmeta_stock_at_term - $item_stock_location_subtract_input_qty );
                                    ...
                                    if( ( ($postmeta_stock - $item_stock_location_subtract_input_qty) <= 0 ) && ($postmeta_stock_status === 'instock') ) {
                                        update_post_meta($item_product_id, '_stock_status', 'outofstock');
                                    } else {
                                        update_post_meta($item_product_id, '_stock_status', 'instock');
                                    }

                                    // Update the itemmeta of the order item
                                    wc_update_order_item_meta($item_id, '_item_stock_locations_updated', 'yes');
                                    wc_update_order_item_meta($item_id, '_item_stock_updated_at_' . $term->term_id, $item_stock_location_subtract_input_qty);
                        ...
                        if($counter === $terms_total) {
                            update_post_meta( $item_product_id, '_stock', $postmeta_stock - array_sum($input_values) );
                        }

                    }

                    // Check if stock in locations are updated for this item
                    if( empty( wc_get_order_item_meta($item_product_id, '_item_stock_locations_updated', true) ) ) {
                        SlwAdminNotice::displayWarning(__('Partial or total stock in locations is missing in this order. Please fill the remaining stock.', 'stock-locations-for-woocommerce'));
                    } else {
